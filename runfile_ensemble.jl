...
#make a depth vector filled with the initial conditions from the setup file. Takes either constants (for steady state runs), or vectors (for perturbations)

function profile(x, rho, Nz)
    if x isa Number
        fill(Float64(x) * rho, Nz)
    elseif PERTURBATION_RUN
...
Nvar    = size(sols[1].u[1])   # assumes at least 1 saved step in trajectory 1
...
# %% [markdown]
# # Save values for restart/perturbations state
...
water_values = Dict{String,Any}(
    ...
    "dtNH4_w" => dtNH4_w,
    "dalk_w" => dalk_w,
    "dalk_alloch_w" => dalk_alloch_w,
    "dalk_aerob_w" => dalk_aerob_w,
    "dalk_anaerob_w" => dalk_anaerob_w,
    "dalk_carb_w" => dalk_carb_w,
    "dtPO4_w" => dtPO4_w,
    "dtH2S_w" => dtH2S_w,
    "dCH4_w" => dCH4_w,
    "dFeII_w" => dFeII_w,
